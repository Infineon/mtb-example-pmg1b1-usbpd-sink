...
#if VBUS_UVP_ENABLE
bool app_psnk_vbus_uvp_cbk (void * cbkContext, bool comp_out);
...
#elif defined(CY_DEVICE_SERIES_PMG1B1)
...
#elif defined(CY_DEVICE_SERIES_PMG1B1)
    Cy_USBPD_Vbus_GdrvPfetOn(context->ptrUsbPdContext, false);
#else
...
void psnk_enable (cy_stc_pdstack_context_t * context)
{
    ...
    /* VBUS UVP will be enabled in BB state machine. */
#if defined(CY_DEVICE_CCG3PA)
    app_status_t* app_stat = app_get_status(context->port);
